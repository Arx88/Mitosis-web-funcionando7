...
# 🚨 PASO 3: CREAR EL CANAL DE COMUNICACIÓN - Task Status Endpoint
@agent_bp.route('/task/<task_id>/status', methods=['GET'])
def get_task_status(task_id: str):
    """
    Endpoint para obtener el estado actual de una tarea específica
    Permite al frontend consultar el progreso de la tarea
    """
    try:
        # 🚨 LOGGING AGRESIVO EN TASK STATUS ENDPOINT
        print(f"🔍 GET /api/task/{task_id}/status called")
        logger.info(f"🔍 Task status requested for: {task_id}")
        
        # Obtener el gestor de memoria avanzada
        from ..memory.advanced_memory_manager import AdvancedMemoryManager
        
        # Crear instancia del gestor de memoria si no existe
        if not hasattr(current_app, 'memory_manager'):
            print(f"🔧 Creating new memory manager instance")
            current_app.memory_manager = AdvancedMemoryManager()
        
        memory_manager = current_app.memory_manager
        print(f"✅ Memory manager obtained")
        
        # Obtener el estado de la tarea
        task_state = memory_manager.get_task_state(task_id)
        print(f"📊 Task state retrieved: {task_state.get('status', 'unknown')}")
        
        # Verificar si la tarea existe
        if task_state.get('status') == 'not_found':
        ...
        if task_state.get('status') == 'error':
            print(f"❌ Error retrieving task state: {task_state.get('error', 'Unknown error')}")
            return jsonify({
        ...
        print(f"✅ Task status endpoint successful")
        return jsonify(task_state), 200
        
    except Exception as e:
        error_msg = f"Error getting task status for {task_id}: {str(e)}"
        logger.error(error_msg)
        print(f"❌ CRITICAL ERROR in task status endpoint: {str(e)}")
